<?php
if (isset($_SERVER['PATH_INFO']) && strpos(strtolower($_SERVER['PATH_INFO']), '/admin') !== false) {
    header("Location: admin/index.php");
    exit;
}
require_once __DIR__ . '/includes/db.php';

// Check if dynamic home page exists and has blocks
$page = null;
try {
    $stmt = $pdo->prepare("SELECT * FROM pages WHERE slug = 'home' AND status='active'");
    $stmt->execute();
    $page = $stmt->fetch();
...
if ($page) {
    // Load sections for the home page
    $sections = [];
    try {
    ...
    if ($sections) {
        ...
        $page_keywords = $page['meta_keywords'] ?? 'home, business';
        if ($page['image']) {
            $page_image = "uploads/" . $page['image'];
        }

        // Handle Contact Form Submission if present on home page
        $success = false;
        $error = '';
        ...
        <?php foreach ($sections as $sec): ?>
          ...
          <?php elseif ($sec['section_type'] === 'services'): ?>
            <?php $services = $pdo->query("SELECT * FROM services WHERE status='active' ORDER BY sort_order LIMIT 6")->fetchAll(); ?>
          ...
          <?php elseif ($sec['section_type'] === 'jobs'): ?>
            <?php $jobs = $pdo->query("SELECT * FROM completed_jobs WHERE status='active' ORDER BY sort_order DESC LIMIT 3")->fetchAll(); ?>
            <section>
              <div class="container">
          ...
          <?php elseif ($sec['section_type'] === 'reviews'): ?>
            <?php $reviews = $pdo->query("SELECT * FROM reviews WHERE status='active' ORDER BY sort_order LIMIT 3")->fetchAll(); ?>
            <section class="bg-alt">
              <div class="container">
...
// Fallback original home page HTML layout
$page_title = "Home";
$services = $pdo->query("SELECT * FROM services WHERE status='active' ORDER BY sort_order LIMIT 6")->fetchAll();
$jobs     = $pdo->query("SELECT * FROM completed_jobs WHERE status='active' ORDER BY sort_order DESC LIMIT 3")->fetchAll();
$reviews  = $pdo->query("SELECT * FROM reviews WHERE status='active' ORDER BY sort_order LIMIT 3")->fetchAll();
...
<section class="hero">
  <div class="container">
    <h1><?= e($settings['site_name'] ?? 'My Business') ?></h1>
    <p><?= e($settings['site_tagline'] ?? '') ?></p>
    <div class="hero-actions">
      <a href="page.php?slug=services" class="btn">Our Services</a>
      <a href="page.php?slug=contact" class="btn btn-outline">Contact Us</a>
    </div>
  </div>
</section>
...
<section>
  <div class="container">
    ...
    <p class="section-sub">What we offer</p>
    <div class="grid">
      <?php foreach ($services as $s): ?>
        <div class="card">
          <?php if ($s['image']): ?><img class="card-img" src="<?= e(img_url($s['image'])) ?>" alt="<?= e($s['title']) ?>"><?php endif; ?>
...
<section class="bg-alt">
  <div class="container">
    ...
    <p class="section-sub">See our latest work</p>
    <div class="grid">
      <?php foreach ($jobs as $j): ?>
        <div class="card">
          <?php if ($j['image']): ?><img class="card-img" src="<?= e(img_url($j['image'])) ?>" alt="<?= e($j['title']) ?>"><?php endif; ?>
...
<section>
  <div class="container">
    ...
    <p class="section-sub">What our clients say</p>
    <div class="grid">
      <?php foreach ($reviews as $r): ?>
        <div class="card review-card">
          <?php if ($r['image']): ?><img class="review-avatar" src="<?= e(img_url($r['image'])) ?>" alt="<?= e($r['client_name']) ?>"><?php endif; ?>
